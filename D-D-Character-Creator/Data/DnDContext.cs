...
using D_D_Character_Creator.Data.Items;
using System;
...
namespace D_D_Character_Creator.Data
{
    public class DnDContext
    {
        ...
        private static string FixQuotes(string exp)
        {
            ...
            if (last < exp.Length) res.Append(exp.AsSpan(last));
            return res.ToString();
        }
        ...
        public void GenerateAbilityScores()
        {
            ...
            Scores.Arrays.AddRange(from AbilityScoreArray a in AbilityScoreArray.Generate()
                                   select a.ToString());
        }

        //Background
        public Dictionary<string, Background> Backgrounds = new(StringComparer.OrdinalIgnoreCase);
        public Dictionary<string, Background> BackgroundsSimple = new(StringComparer.OrdinalIgnoreCase);
        public Background GetBackground(string name, string sourcehint)
        {
            ...
            if (sourcehint != null
                && Backgrounds.ContainsKey(name + " " + ConfigManager.SourceSeperator + " " + sourcehint)) return Backgrounds[name + " " + ConfigManager.SourceSeperator + " " + sourcehint];
        ...
        //Classes
        public Dictionary<string, ClassDefinition> Classes = new(StringComparer.OrdinalIgnoreCase);
        public Dictionary<string, ClassDefinition> ClassesSimple = new(StringComparer.OrdinalIgnoreCase);
        ...
        public ClassDefinition GetClass(String name, string sourcehint)
        {
            ...
            if (sourcehint != null
                && Classes.ContainsKey(name + " " + ConfigManager.SourceSeperator + " " + sourcehint)) return Classes[name + " " + ConfigManager.SourceSeperator + " " + sourcehint];
        ...
        {
            return level > 1
                ? (from c in Classes.Values where provider.CanMulticlass(c, level) select c)
                : (from c in Classes.Values where c.AvailableAtFirstLevel select c);
        }

        //Conditions
        public Dictionary<string, Condition> Conditions = new(StringComparer.OrdinalIgnoreCase);
        public Dictionary<string, Condition> ConditionsSimple = new(StringComparer.OrdinalIgnoreCase);
        public Condition GetCondition(string name, string sourcehint)
        {
            ...
            return sourcehint != null
                && Conditions.ContainsKey(name + " " + ConfigManager.SourceSeperator + " " + sourcehint)
                ? Conditions[name + " " + ConfigManager.SourceSeperator + " " + sourcehint]
                : ConditionsSimple.ContainsKey(name) ? ConditionsSimple[name] : new Condition(this, name, "");
        }
        ...
        //FeatureCollections
        public List<Dictionary<string, List<Feature>>> FeatureCollections = new();
        public Dictionary<string, List<FeatureContainer>> FeatureContainers = new(StringComparer.OrdinalIgnoreCase);
        public Dictionary<string, List<Feature>> FeatureCategories = new(StringComparer.OrdinalIgnoreCase);
        public Dictionary<string, Feature> Boons = new(StringComparer.OrdinalIgnoreCase);
        public Dictionary<string, Feature> BoonsSimple = new(StringComparer.OrdinalIgnoreCase);
        public List<Feature> Features = new();
        private readonly List<List<Feature>> copies = new();
        public List<Feature> GetFeatureCollection(string expression, int copy = 0)
        {
            ...
            if (expression is null or "") expression = "Category = 'Feats'";
            if (expression
                == "Boons") expression = "Category = 'Boons'";
                ...
                ex.EvaluateParameter += delegate (string name, ParameterArgs args)
                {
                    ...
                    switch (name)
                    {
                        ...
                        case "source":
                            ...
                            break;
                        default:
                            if (current.Keywords.Count > 0 && current.Keywords.Exists(k => k.Name == name)) args.Result = true;
